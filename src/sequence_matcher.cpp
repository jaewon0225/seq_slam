...
cv::Mat calculateDifferenceMatrix(const std::vector<cv::Mat>& map_images, const std::vector<cv::Mat>& query_images) {
...
std::vector<int> findOptimalPath(const cv::Mat& enhanced_matrix, int sequence_length) {
    ...
    for (int i = 1; i < n; i++) {
        ...
        for (int j = 0; j < m; j++) {
            for (int k = std::max(0, j - sequence_length); k <= std::min(m - 1, j + sequence_length); k++) {
